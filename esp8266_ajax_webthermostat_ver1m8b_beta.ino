...
// if webpage is without client(s)
 ...
 if (millis() > taim5 + 25000) // if time between sensor reading is too lonf 
{
...
if (path.startsWith("RedLED"))
              {
              Serial.println(" SUMIT button..."); 
              }            
if (path.startsWith("/ajax_switch&nocache="))      
//if (path.startsWith("/ajax_switch"))      
              {
...
if ((te >= teset - dete/2) and (te <= teset + dete/2))
{
  ...
  connection->send(F("<font color=green> as last state (temperature is OK!)"));
}
...
/*else if (path.startsWith("RedLEDState="))
              {
              Serial.println("SUMIT button..."); 
              }
*/              
else if (path == "/"){
//else if ((path.indexOf("GET / ") != -1) || (path.indexOf("POST / ") != -1)) {
             connection->send(F(
                "HTTP/1.1 200 OK\r\n\r\n"
                "<!DOCTYPE html>\r\n"
                "<html>\r\n"         
...
connection->send(F(
 "<form action=\"\" method=\"post\">"
                         "<fieldset>"
                            ...
                            "<input type=\"radio\" name=\"RedLEDState\" value=\"RED_6\"> 6<sup>0</sup>C    "
                            "<input type=\"radio\" name=\"RedLEDState\" value=\"RED_7\"> 7<sup>0</sup>C    "
                            "<input type=\"radio\" name=\"RedLEDState\" value=\"RED_8\"> 8<sup>0</sup>C    "
                            "<input type=\"radio\" name=\"RedLEDState\" value=\"RED_9\"> 9<sup>0</sup>C    "
                            "<input type=\"radio\" name=\"RedLEDState\" value=\"RED_10\">10<sup>0</sup>C    "
                            "<input type=\"radio\" name=\"RedLEDState\" value=\"RED_11\">11<sup>0</sup>C    "
    ...
    //            "<FORM><INPUT Type=\"BUTTON\" Value=\"http://arduinotehniq.blogspot.com\" Onclick=\"window.location.href='http://arduinotehniq.blogspot.com' target=_blanck\"></FORM>"
               "<p><br>version. 1.8.b - 21.11.2015, Craiova</br>"
                "</body>\r\n"          
                "</html>\r\n"));               
               }
